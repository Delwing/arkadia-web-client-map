...
describe('stun triggers', () => {
  ...
  test('golem stun does not duplicate prefix if already present', () => {
    const lineWithPrefix = '[OGLUCH] golem w mgnieniu oka uderza w Bob, a on wyrwany z oslupienia, probuje ratowac sie krokiem w tyl. Jednak wiele to nie pomaga i sila uderzenia odrzuca';
    const result = parse(lineWithPrefix);
    // Count occurrences of [OGLUCH] - should only be 1
    const matches = (result.match(/\[OGLUCH\]/g) || []).length;
