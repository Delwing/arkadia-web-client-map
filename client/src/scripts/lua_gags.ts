    ...
    tr.patterns.forEach((pat) => {
        ...
        const callback = (rawLine: string, _, matches: RegExpMatchArray) => {
            if (tr.script != undefined) {
                evaluatedLine = rawLine
                luaEnv.parse("line = \"" + rawLine + "\"").exec()
                luaEnv.parse(createMatchesScript(matches)).exec()
                luaEnv.parse(tr.script).exec()
                rawLine = gagPrefix(rawLine, evaluated, evaluatedType)
            }
            return rawLine;
        }
...
const ateam = {
    may_setup_paralyzed_name: (_, name: string) => console.log(name),
}
...
Object.entries(mudlet).forEach(([key, value]) => {
...
// Dynamically import all Lua files in the 'lua/' directory
// @ts-ignore
const luaFiles = import.meta.glob("../lua/**/*.lua", {query: "?raw", eager: true});
Object.entries(luaFiles).forEach(([path, file]) => {
    // @ts-ignore
    luaEnv.parse(file.default).exec()
});


function createMatchesScript(matches: RegExpMatchArray) {
