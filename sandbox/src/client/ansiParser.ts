...
function handleColorChange(
    ...
    if (colorIndex >= 0 && colorIndex < COLOR_TABLE.length) {
        activeColors.push(COLOR_TABLE[colorIndex]);
...
export function parseAnsiPatterns(input: string): string {
    ...
    for (let match; match = ansiRegex.exec(input);) {
        ...
        if (ansiSequence === "0" || ansiSequence === "39;22") {
            ...
            handleColorChange(activeColors, spanStartIndices, spanEndIndices, currentSpanCount, matchPos, colorIndex);
